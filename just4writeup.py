...
def single_img_features(img):
    ...
    if hog_feat == True:
        if hog_channel == 'ALL':
            ...
            hog0,hog0_img = get_hog_features(feature_image[:,:,0], vis=True, feature_vec=True)
            hog_features.extend(hog0)

            hog1,hog1_img = get_hog_features(feature_image[:,:,0], vis=True, feature_vec=True)
            hog_features.extend(hog1)

            hog2,hog2_img = get_hog_features(feature_image[:,:,0], vis=True, feature_vec=True)
            hog_features.extend(hog2)

            # Plot for writeup!
            fig = plt.figure()
            plt.subplot(131)
            plt.imshow(hog0_img)
            plt.title('hog feature for channel 0')
            plt.subplot(132)
            plt.imshow(hog1_img)
            plt.title('hog feature for channel 1')
            plt.subplot(133)
            plt.imshow(hog2_img)
            plt.title('hog feature for channel 2')
            plt.show()
...
def car_detect(img):
    ...
    for bbox in box_list:
        cv2.rectangle(img_before, bbox[0], bbox[1], (0,0,255), 6)

    #AFTER
    img_after = np.copy(img)
    img_after = draw_labeled_bboxes(img_after,labels)
